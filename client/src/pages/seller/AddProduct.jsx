...
const AddProduct = () => {
    ...
    const onSubmitHandler = async(e) => {
        try{
            ...
            const {data} = await axios.post('https://greencart-mk3l.onrender.com/api/product/add', formData)
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className="max-w-24 cursor-pointer" src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} alt={`Product ${index + 1}`} />
                            </label>
                        ))}
                    </div>
                </div>
